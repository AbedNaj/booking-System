...
<div
    ...
    @php
        $status = App\Enums\BookingStatusEnum::tryFrom($data->status);
    @endphp
    <div class="mt-3 flex justify-between items-center">
        <span
            class="inline-flex items-center px-2 py-1 text-xs rounded-lg font-medium bg-{{ $status->color() }}-100 text-{{ $status->color() }}-800">
        ...
        <div class="flex space-x-2">
            <a href="{{ route('app.employee.booking.details', ['booking' => $data->id]) }}"
                class="text-blue-500 hover:text-blue-700 text-sm font-medium hover:cursor-pointer">{{ __('booking.deatils') }}</a>
