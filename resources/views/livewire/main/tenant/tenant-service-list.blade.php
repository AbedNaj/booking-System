<div>
    <section class="max-w-7xl mx-auto px-4 py-16">
      ...
      <div class="text-center mb-16">
        <h1 class="text-5xl font-extrabold mb-6 bg-gradient-to-r from-blue-600 to-purple-600 inline-block text-transparent bg-clip-text">
     {{$tenants->name}}
        </h1>
  ...
  @foreach ($categories as $category )
          ...
          wire:click="filterServices({{ $category->id }})"
          :class="{'bg-blue-600 text-white': activeFilter === {{ $category->id }}, 'bg-gray-100 text-gray-800': activeFilter !== {{ $category->id }}}"
          class="px-6 py-2 rounded-full font-medium transition-all duration-300 hover:shadow-md">
     {{ $category->name }}
        </button>
  @endforeach
  ...
  @foreach ($services as $service)
     <x-main.service-card :data="$service"></x-main.service-card>
  @endforeach
      ...
      <div class="mt-12 text-center">
        <button class="inline-flex items-center bg-white border border-blue-600 text-blue-600 hover:bg-blue-50 font-medium rounded-lg px-8 py-3 transition-all duration-300">
      ...
      <div class="mt-24">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div class="bg-white p-6 rounded-xl shadow-md flex flex-col items-center text-center">
            ...
            <p class="text-gray-600">نستخدم فقط منتجات ذات جودة عالية وأدوات معقمة لضمان أفضل النتائج والحفاظ على صحتك.</p>
          </div>
          <div class="bg-white p-6 rounded-xl shadow-md flex flex-col items-center text-center">
            <div class="bg-blue-100 p-3 rounded-full mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div class="bg-white p-6 rounded-xl shadow-md flex flex-col items-center text-center">
            ...
            <p class="text-gray-600">نقدم أسعاراً مناسبة مع عروض وخصومات مستمرة لنضمن لك قيمة مقابل كل شيكل تدفعه.</p>
          </div>
        </div>
      </div>
    </section>
